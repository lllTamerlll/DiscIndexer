...
namespace Cost_Calculation.Services
{
    public static class AutoMarkService
    {
        ...
        {
            <= 3 => Tuning.AutoMarkCutoffSmall,
            4 => Tuning.AutoMarkCutoffMedium,
            _ => Tuning.AutoMarkCutoffLarge
        };
            ...
            Action<int, int>? report)
        {
            ...
            foreach (var items in groups)
            {
                ...
                // Лучший результат группы не помечается никогда — даже если
                // таких дисков несколько (полностью равные по пользе диски не
                // должны отправляться на распыление из-за позиции в сортировке).
                int bestScore = scores[sorted[0]];
                for (int i = 1; i < sorted.Count; i++)
                {
                    int score = scores[sorted[i]];
